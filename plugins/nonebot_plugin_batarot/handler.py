...
async def handle_daily_fortune(event: MessageEvent):
    cards_dict, tarot_urls = load_tarot_data()

    now = datetime.now()
    date_str = now.strftime("%Y-%m-%d")
    seed = date_str+event.get_user_id()

    fortune = hashlib.sha256(f'{seed}4396'.encode()).hexdigest()
    fortune_score = int(fortune, 16) % 100 + 1

    if event.get_user_id() == "924740926":
        if fortune_score % 10 == 0:
            fortune_score = 100
        else:
            fortune_score =  fortune_score % 10 + 90
    ...
    ck = hashlib.sha256(f'{seed}223'.encode()).hexdigest()
    ck_list=list(cards_dict.keys())
    ck_val = int(ck, 16) % len(ck_list)

    card_key = ck_list[ck_val]
    card = cards_dict[card_key]
    card_name = card['name_cn']
    card_url = tarot_urls.get(f"tarot_{card_key}")
    ...
    fortune_description_range = fortune_descriptions[score_range]
    fortune_description=fortune_description_range[fortune_score % len(fortune_description_range)]
